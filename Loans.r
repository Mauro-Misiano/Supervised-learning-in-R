...
## Variable de respuesta como factor
...
table(datos$deny,datos$mhist) #podria recategorizarse juntando 3 y 4
table(datos$deny,datos$phist)
table(datos$deny,datos$insurance) #casi todos los valores son "no"
...
#comparo los 3 modelos segun AIC:
akaike<-AIC(mod1,mod2,mod3)

#4.#elijo modelo 3 porque tiene menor AIC
options(digits=6) #para evitar notacion cientifica
summary(mod3)
# a)	¿Es significativo el modelo?
modeloNULL<- glm(deny ~ 1, family = "binomial", data =train)
...
#selfemp poco!! tampoco lo es mhist3 
...
# mejorar punto de corte evaluando accu
